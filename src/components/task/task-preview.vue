  ...
  <div class="task-preview flex" @mouseleave="delHover = false">
    ...
    <div class="task-title-comp">
      ...
      >
        ...
        <span class="chat">
          <span class="comments-indicator" v-if="task.comments?.length">{{ task.comments.length }} </span>
          <svg
            viewBox="0 0 20 20"
            fill="currentColor"
            width="24"
            height="24"
            aria-hidden="true"
            :class="[task.comments?.length>0? ['on', 'chat-without-update']:'chat-without-update']"
          >
            <path
