<template>
  <article class="flex relative">
    ...
    <div class="ellipsis flex align-center space-evenly b-wrapper">
      ...
      <span class="flex align-center" :class="{'d-none' : !isHover}" 
        ><img
          src="../assets/img/ellipsis-solid.svg"
          alt=""
          class="elipsis-board absolute"
          @click.stop="isOptions = !isOptions"
        />
      </span>
    </router-link>
      </div>
...
export default {
  ...
  methods: {
    setBoardUpdate(val) {
      this.$emit("setBoardUpdate", val, this.board);
    },
    updateBoard($event, board) {
      this.$emit("updateBoard", $event.target.innerText, board);
    },
    chooseBoard(board) {
